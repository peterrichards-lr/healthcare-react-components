...
const HEART_RATE_API_PATH = 'o/c/weights/';
const USER_ID_FIELD = 'r_weight_userId';
const READING_DATE_FIELD = 'readingDate';

const weightApi = (startDate, endDate, maxEntries) => {
  console.debug(`Param startDate=${startDate}`);
  console.debug(`Param endDate=${endDate}`);
  console.debug(`Param maxEntries=${maxEntries}`);

  startDate =
    startDate !== undefined && startDate instanceof Date
      ? startDate
      : new Date();
  endDate =
    endDate !== undefined && endDate instanceof Date
      ? endDate
